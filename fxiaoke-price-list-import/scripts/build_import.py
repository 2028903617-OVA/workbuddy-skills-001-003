...
# 模糊匹配的两道闸（2026-09-15 实战错配教训）：
#   案例1 东芝…100Z600QFR-S 错配 索尼…100Z600QF → 品牌闸
#   案例2 美的波轮洗衣机MB10V56T 错配 产品名就叫「波轮洗衣机」→ 候选无型号码闸
#   案例3 美的电饭煲MB-AFB40C8 错配 并存老产品 美的电饭煲AFB40C8 → 边界闸（'-' 不算干净边界）
BRAND_WORDS = ["A.O.史密斯", "史密斯", "卡萨帝", "COLMO", "小天鹅", "西门子", "博世", "博士",
...
def brand_words(s):
    s = str(s or "")
    return {b for b in BRAND_WORDS if b in s}


def model_tokens(s):
    """型号码 token：含数字、长度>=4 的字母数字段（可含 - . / +）"""
    return [t for t in re.findall(r"[A-Za-z0-9][A-Za-z0-9\-\.\+/]{3,}", str(s or ""))
            if re.search(r"\d", t)]


def fuzzy_ok(model, cand_name):
    """非精确匹配（型号包含/子串）的两道闸。返回 (ok, 原因)。"""
    bs, bc = brand_words(model), brand_words(cand_name)
    if bs and bc and not (bs & bc):
        return False, "品牌不一致"
    mt, ct = model_tokens(model), model_tokens(cand_name)
    if not ct:
        return False, "候选产品名无型号码"
    for t in ct:
        i = model.find(t)
        if i < 0:
            return False, "型号码 %s 不在源表型号中" % t
        pre = model[i - 1] if i > 0 else ""
        post = model[i + len(t)] if i + len(t) < len(model) else ""
        if (pre and _DIRTY_EDGE.match(pre)) or (post and _DIRTY_EDGE.match(post)):
            return False, "型号码 %s 边界不完整(%s|%s)" % (t, pre, post)
    # 括号变体（如 (HE)）必须两边都有：源表有而候选无 → 不同产品
    m_par = re.search(r"\([^)]*\)", str(model))
    if m_par and m_par.group(0) not in str(cand_name):
        return False, "源表含括号变体 %s，候选没有" % m_par.group(0)
    c_par = re.search(r"\([^)]*\)", str(cand_name))
    if c_par and c_par.group(0) not in str(model):
        return False, "候选含括号变体 %s，源表没有" % c_par.group(0)
    return True, ""
