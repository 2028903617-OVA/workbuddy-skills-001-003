...
**现象**：Office / WPS 窗口强制置顶，挡住其他所有窗口。

**根因**：软件侧的窗口管理策略，**不是脚本能改的**。

✅ 只能在软件设置里关（WPS：设置 → 切换窗口管理；Office：各组件选项里关置顶），**提醒用户去设置里关即可**。
❌ 不要试图用脚本抢焦点或置前台——越抢越乱，还会干扰用户正在做的操作。

**举一反三**：先分清"程序能改的"和"只能让人去设置的"，后者不要浪费轮次去绕。

## 2.4 ★ 读外部导出的 Excel 一律 `read_only=False`（B05）
...
## 2.17 ★ Office 里的中文常被写成 XML 数字实体（B15）

**现象**：想确认 xlsx 里有没有某个中文词，解压后直接搜 XML，**搜不到**——可单元格里明明有。

**根因**：xlsx / docx 是 OOXML 压缩包，中文在 XML 里常被写成数字实体：
...
✅ 要么**用库读值**（openpyxl / python-docx），要么解压后先 `html.unescape` 再搜。
⚠️ 这条直接影响**脱敏自检**：只解压搜中文 = 假阴性，会误以为文件是干净的（C04）。

**举一反三**：凡是"压缩包里找文本"，都要先问一句"文本是明文吗、有没有被转义"。
