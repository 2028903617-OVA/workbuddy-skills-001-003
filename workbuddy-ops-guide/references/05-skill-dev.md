...
- 全包无绝对路径残留（E04：**脚本只写相对路径**）。注意本机 bash 的 `grep` 不可用（A01），用 Python 查：

```python
import os, re, io
root = r'<skill 目录>'
pat = re.compile(r'[A-Za-z]:[\\/](?:Users|home)[\\/]|/home/')
for r, _, fs in os.walk(root):
    for f in fs:
        ...
        for i, line in enumerate(t.split('\n'), 1):
            if pat.search(line):
                print(p, i, line.strip()[:100])
```
- **本机专用文件已摘除**：任何写着本机路径、机器标签、排程时刻的文件，分享前删掉或清空（见 `00-my-machine.md` 的说明）
- 分享前再跑业务词脱敏（C04）
...
❌ 反例：备份是"改完代码、清文档之前"的中途快照，SKILL.md 仍是旧措辞 → **无法回滚到上一版**。
...
⚠️ **E08**：记忆枢纽只适合文档型，**整目录镜像会撑爆**（单节点过大触发网关超时、目录结构被破坏）。
...
```cmd
xcopy "%USERPROFILE%\.workbuddy\skills" "<备份目录>\" /E /I /Y
```

### 排程
- 两机时间**错开约 30 分钟**避免并发冲突（具体时刻记在 `00-my-machine.md`）
- 对端关机不影响（拉的是已推送的版本）
- 排程要落在**开机窗口内**，留 30 分钟余量

### 超大 skill 走手动（被 .gitignore 排除）
体积过大的目录 git 不传 → 打包 zip（**跳过 `.git`**）手工拷一次。

### 同步后验证
跑同步脚本看状态，应显示本机 / 对端身份、远端已配置、工作区 clean、无冲突。
**跨机闭环 = "在 A 上做 + 在 B 上验证"**，缺 B 验证就是半截流程。

### ⚠️ E13 记忆同步窗口保持短期
只处理**最近几天的每日文件 + 全局 MEMORY.md**。改回全量遍历会把共享节点撑爆——**改一次事故一次**。长节假日才临时放宽窗口。
...
wmic os get Version
```
全部吻合才继续。锚点值记在 `00-my-machine.md`。

## 5.7 源表解析的两类坑（E06 / E07）

### E06 源表版式识别走错分支
**现象**：同一类源表有多种版式（列位不同 / 有无分组行 / 合并表头），脚本按单一版式解析，遇到另一种版式静默走错分支，结果错还不报错。

**根因**：把"上一次见到的版式"当成唯一版式。

✅ 解析前先做**版式探测**（按表头关键字 / 列数 / 有无合并单元格分派），每种版式一个分支；
**遇到不认识的版式要明说"不支持"并给出人工绕过路径**，不许猜着往下走。

### E07 多 sheet 全量合并被纠
**现象**：源里同一类数据有多个 sheet（按月份 / 门店分表），脚本全部合并，把历史期间也算进去，被纠正。

**根因**：默认"全都要"，没问清口径。

✅ 默认**只取最新**（6 位日期取最大；无法比较时取最右）；**要全量合并必须用户明确确认**。
合并前先打印 sheet 清单与各自归属期间，让用户挑。
